			...
			synchronized(this) {
				...
				if (aircraft.isRunway() || aircraft.isParked() || !checkForNext) {
					for (Aircraft a : aircraftContainer.getArrayList()) {
						if (!(a.isRunway()) && (aircraft.getID() != a.getID())) {
							if (!a.isParked()) {
								aircraft = a;
									// Main.addLog(String.format("%s", aircraft.getStatus()));
								break;
							}
						}
					}
				...
				synchronized(aircraft) {
					...
					if (aircraft.isFlying()) {
						...
						Main.addLog(String.format("[ID: %d] (2/5) %s is landing at Runway %d in 10seconds", aircraft.getID(), aircraft.getName(), this.getID()));
						...
						Main.addLog(String.format("[ID: %d] (3/5) %s finished landing at Runway %d and is parked in the airport for %dseconds", aircraft.getID(), aircraft.getName(), this.getID(), parkTime/1000));
				...
				// synchronized(aircraft) {
					if (aircraft.isLanded() && checkForNext) {
						...
						Main.addLog(String.format("[ID: %d] (4/5) %s is taking off from Runway %d to %s in %dseconds", aircraft.getID(), aircraft.getName(), this.getID(), aircraft.getDestination(), takeOffTime));
						...
						for (int i = 0; i < aircraftContainer.getArrayList().size(); i++) {
							if (aircraftContainer.getArrayList().get(i).getID() == aircraft.getID()) {
								...
								Main.addLog(String.format("[ID: %d] (5/5) %s has flown off from Runway %d to %s", aircraft.getID(), aircraft.getName(), this.getID(), aircraft.getDestination()));
								break;
							}
						}
					}
